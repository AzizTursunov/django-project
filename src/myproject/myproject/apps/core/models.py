...
def object_relation_base_factory(
    ...
    add_related_name: a boolean value indicating, that
                      a related name for the generated content
                      type foreign key should be added. This
                      value should be true, if you use more
                      than one ObjectRelationBase in your model.
    ...
    if add_related_name:
        if not prefix:
            raise FieldError(
                'if add_related_name is set to "True", '
